...
export const getAvailableTimes = actionClient
  ...
  .action(async ({ parsedInput }) => {
    ...
    const appointmentsOnSelectedDate = appointments
      ...
      .map((appointment) => dayjs(appointment.date).format("HH:mm"));
    ...
    const doctorTimeSlots = timeSlots.filter((time) => {
      ...
      const date = dayjs()
        .utc()
        .set("hour", Number(normalizedTime.split(":")[0]))
        .set("minute", Number(normalizedTime.split(":")[1]))
        .set("second", 0);
    ...
    return doctorTimeSlots.map((time) => {
      ...
      return {
        ...
        available: !appointmentsOnSelectedDate.includes(normalizedTime),
